...
#pragma warning restore 0414
        ...
        public List<KeyValuePair> GetKeyValuePairList() { return list; }

        public void CopyFrom(GenericDictionary<TKey, TValue> _other)
        {
            if (_other != null)
            {
                dict.Clear();
                ...
                for (int i = 0; i < _other.list.Count(); i++)
                {
                    TKey _key = _other.list[i].Key;
                    TValue _value = _other.list[i].Value;

                    dict.Add(_key, _value);
                    list.Add(_other.list[i]);
                    indexByKey.Add(_key, i);
                }
            }
            else
            {
                Debug.Log("Cancelled the attempt to copy from a Null GenericDictionary");
            }
        }
